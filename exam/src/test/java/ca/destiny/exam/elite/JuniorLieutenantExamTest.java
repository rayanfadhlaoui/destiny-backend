package ca.destiny.exam.elite;

import ca.destiny.ApplicationTest;
import ca.destiny.exam.AbstractExamTest;
import ca.destiny.exam.BattleConfiguration;
import ca.destiny.exam.advance.OfficerExam;
import org.junit.jupiter.api.Test;
import org.junit.jupiter.api.extension.ExtendWith;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.context.annotation.Import;
import org.springframework.test.context.junit.jupiter.SpringExtension;

import java.io.IOException;
import java.net.URISyntaxException;

@ExtendWith(SpringExtension.class)
@Import(BattleConfiguration.class)
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.MOCK, classes = ApplicationTest.class)
public class JuniorLieutenantExamTest extends AbstractExamTest {

    public JuniorLieutenantExamTest(@Autowired JuniorLieutenantExam juniorLieutenantExam) {
        super(JuniorLieutenantExamTest.class, juniorLieutenantExam);
    }

    @Test
    void juniorLieutenantsExam() throws IOException, URISyntaxException {
        exam("officers.json", "juniorLieutenants.json", "officers.json", 150);
    }
}
